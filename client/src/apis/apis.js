...
function processBookings(data) {
  ...
  const processedBookings = data.invited_details.map((item) => {
...
function processBookingsMulti(data) {
  console.log("data", data);
  const processedBookings = data.hosted_bookings_map.map((item) => {
    const [date, start_time] = item.start_time.split(" ");
    const { rooms } = item;
...
export const getCreatedBookingsMulti = async (userId) => {
  try {
    ...
    const processedData = processBookingsMulti(response.data);
    return processedData;
  } catch (e) {
    throw e;
  }
...
export const getActive = async () => {
  try {
    const response = await fetch(`${EMPLOYEE_BASE_URL}/getActive`);
    return response;
  } catch (e) {
    throw e;
  }
