...
function xyDistance(room1, room2) {
...
async function kmeansPlusPlus(participantsWithDetails, numClusters) {
  ...
  for (let k = 1; k < numClusters; k++) {
    ...
    for (let i = 0; i < latitudeLongitudeFloor.length; i++) {
      ...
      closestDistances[i] = closestDistance ** 2;
    }
    // next centroid
...
async function reCluster(dataPoints, assignments, numClusters, centroids) {
  ...
  for (let i = 0; i < dataPoints.length; i++) {
    ...
    sumCoordinates[centroidIndex].latitude += parseFloat(point.latitude);
    sumCoordinates[centroidIndex].longitude += parseFloat(point.longitude);
    sumCoordinates[centroidIndex].floorNumber += parseFloat(point.floorNumber);
    ...
    try {
      ...
      getParticipantDetails(participants, dbConnection)
        .then(async (participantsWithDetails) => {
          ...
          if (emptyCheck >= 0) {
            for(let i =0;i<emptyClusterCheck.length;i++){
              if(emptyClusterCheck[i]===0){
                const borrowingIndexes = centroids.map((cen,ind)=>({cen,ind})).filter((item)=>emptyClusterCheck[item.ind]>1);
                const distances = await Promise.all(borrowingIndexes.map((item)=>distance(centroids[i],item.cen)));
                let minDist = Infinity;
                let borIndex = -1;
                for(let j=0;j<distances.length;j++){
                  if(distances[j]<minDist){
                    minDist = distances[j];
                    borIndex = borrowingIndexes[j].ind;
                  }
                }

                const potentialReassigns = participantsWithDetails.filter((participant)=>participant.closestCentroidIndex===borIndex+1);
                const finalReassign = potentialReassigns[Math.floor(Math.random()*potentialReassigns.length)];
                finalReassign.closestCentroidIndex = i+1;
                emptyClusterCheck[i]++;
                emptyClusterCheck[borIndex]--;
              }
            }
          }
